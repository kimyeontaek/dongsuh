...
public interface CsBoardDAO {
   ...
   /* 제목눌러서 문의 상세보기 - 한 개의 Row 가져오기 */
    ...
    //선택 삭제하기
      ...
      public int getSelectDeleteResult(CsBoardVO vo);/*{
         int result = 0;
         
         try{
            String sql = "delete from cs_board where no = ?";
            getPreparedStatement(sql);
            for(int i = 0; vo.getCsBoard_checkbox().length>i; i++){
            System.out.println(vo.getCsBoard_checkbox()[i]);
            pstmt.setString(1, vo.getCsBoard_checkbox()[i]);
      ...
      public int execSerchCount(String serch_word, String sid);/*{
         int result =0;
         try{
            String sql = "select count(*) from cs_board where content like ? and name =?";
            getPreparedStatement(sql);
            pstmt.setString(1,"%"+serch_word+"%");
      ...
      /* 개인(sid) 전체 리스트 */
      public ArrayList<CsBoardVO> getResultList2(int startCount, int endCount, String sid);
    ...
    //ID 중복체크
          ...
          //게시판 검색하기 리스트 셀랙트
            public ArrayList<CsBoardVO> getSerchResultList(int startCount, int endCount, String serch_word, String sid);
            
          //게시판 검색하기 리스트 셀랙트_관리자
            public ArrayList<CsBoardVO> getSerchResultList2(int startCount, int endCount, String serch_word);
