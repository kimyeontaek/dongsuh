...
$(document).ready(function(){
   //전체 선택 체크박스 ver
    ...
    //전체 선택 버튼 ver (html에 value 값을 추가해서 가져와서 true면 체크하고 다시 false로 바꿔줌.)
    $("#all").click(function(){
        if($("#all").val()=="true"){
            $("input[name=csBoard_checkbox]").prop("checked",true);
            $("#all").val("false");
   ...
   $(document).ready(function(){
      ...
      jQuery('#ampaginationsm').on('am.pagination.change',function(e){
            ...
            jQuery('.showlabelsm').text('The selected page no: '+e.page);
                 $(location).attr('href', "http://localhost:8080/dongsuh/csAdmin_serch_controller.do?serch_word=${serch_word}&rpage="+e.page);         
...
<div id="cs_div1">
   <div id="cs_div2">
   <p id = "slash_red">/</p>
   <p id = "cs_title">[관리자용] 전체 문의 내역</p>
   <p class="wave"><img src="http://localhost:8080/dongsuh/cs_image/bg_tit_wave.png"></p>
   </div>
      <form name="csBoard_serch" action ="http://localhost:8080/dongsuh/csAdmin_serch_controller.do" method = "get">
         <span id = rjator>
         <input type="text" name = serch_word placeholder ="내용 검색" style="height : 34px;">               
         <button type="submit"  class="myButton">검색</button></span>
      </form>
      <form name="csBoard_list" action ="http://localhost:8080/dongsuh/csAdminForm_select_controller.do" method = "post">
      ...
      <tr>
         ...
         <c:choose>
             <c:when test="${empty vo.fname}">
                 <td id = "csBoard_left">
               <a href = "http://localhost:8080/dongsuh/csAdminBoard_content.do?no=${vo.no}&rpage=${rpage}">
               ${vo.content } ...더보기</a>
               </td>
             </c:when>
             <c:when test="${!empty vo.fname}">
   ...
   <a href = "http://localhost:8080/dongsuh/csAdminBoard_list.do">
